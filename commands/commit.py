...
def get_key_from_config(
    ...
    if not Path(config_path).is_file():
        print("Configuration file not found. Please run 'sccs init <file_path>' to init"
...
def get_commit_history():
    ...
    if not Path(history_path).is_file():
        print("History file not found. Please run 'sccs init <file_path>' to initialize"
        " SCCS for this file.")
        sys.exit(1)
...
def update_commit_log_history(
    ...
    history["history"]["commit_order"][latest_commit_number] = (
        f"{sha_hash}"
    )
...
def update_commit_messages(
    ...
    if not Path(commit_messages_path).is_file():
        print("Commit messages file not found. Please run 'sccs init <file_path>' to in"
...
def update_commit_binary_hash_history(
    ...
    if not Path(commit_file_hash_path).is_file():
        print("Commit file hash not found. Please run 'sccs init <file_path>' to initia"
...
def print_commit_confirmation_message(
        sha_hash
    ):
