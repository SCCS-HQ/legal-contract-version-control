...
def get_matching_file_paths(c: SCCSConstants, repo: RepositoryLayout, filename: str) -> list:
    """
...
def push_GET(c: SCCSConstants, repo: RepositoryLayout) -> requests.Response:
    """Make a GET request to 'remote'/push, returning the response."""

    url = c.PUSH_ENDPOINT_TEMPLATE.format(base_url=repo.base_repo_url())
    try:
...
def zip_files_to_upload(c: SCCSConstants, repo: RepositoryLayout, remote_objects: list) -> io.BytesIO:
    """
    ...
    objects_paths = [
        i.resolve()
        for i in (repo.objects_path()).rglob(c.RGLOB_ALL_FILES_PATTERN)
        if i.is_file() and i.stem in obj_to_upload_set
    ]

    history_paths = get_matching_file_paths(c, repo, c.HISTORY_DIR)
    byte_hash_paths = get_matching_file_paths(c, repo, c.COMMIT_FILE_HASH_DIR)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        tmp_folder_path = Path(temp_dir) / c.TMP_DIR_TEMPLATE.format(repo_name=repo.repo_name)
...
def push_POST(c: SCCSConstants, repo: RepositoryLayout, buffer: io.BytesIO) -> requests.Response:
    """
    ...
    if not remote_path.endswith(c.REQUIRED_PATH_ENDING_TEMPLATE.format(repo_name=repo.repo_name)):
...
def clear_updated_branches(c: SCCSConstants, repo: RepositoryLayout) -> None:
    ...
    try:
        with open(repo.current_branch_path(), "w", encoding=c.UTF_8, newline=c.NEWLINE) as f:
...
def main(c: SCCSConstants, repo: RepositoryLayout) -> None:
    ...
    GET_response = push_GET(c, repo)

    GET_response.raise_for_status()

    remote_objects = GET_response.json()[c.HTTP_OBJECTS_DICT_KEY]
    
    buffer = zip_files_to_upload(c, repo, remote_objects)

    POST_response = push_POST(c, repo, buffer)

    POST_response.raise_for_status()

    clear_updated_branches(c, repo)
